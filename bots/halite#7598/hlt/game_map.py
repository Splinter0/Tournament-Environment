...
class Map:
    ...
    def getCorners(self):
        self.corners = [
            entity.Entity(0, 0, 0.5, 100, self.get_me(), 3), #top-left
            entity.Entity(self.width, 0, 0.5, 100, self.get_me(), 4), #top-right
            entity.Entity(0, self.height, 0.5, 100, self.get_me(), 5), #bottom-left
            entity.Entity(self.width, self.height, 0.5, 100, self.get_me(), 6) #bottom-right
        ]
        return self.corners

    def getWalls(self, ship):
        top = entity.Entity(ship.x, self.height, 0.5, 100, self.get_me(), 1)
        bottom = entity.Entity(ship.x, 0, 0.5, 100, self.get_me(), 2)
