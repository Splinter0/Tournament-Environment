...
class Ship(Entity):
    ...
    def __init__(self, player_id, ship_id, x, y, hp, vel_x, vel_y,
                 docking_status, planet, progress, cooldown, velocity):
        self.id = ship_id
        self.x = x
        self.y = y
    ...
    def navigate(self, target, game_map, speed, avoid_obstacles=True,
                 max_corrections=constants.MAX_CORRECTIONS,
                 angular_step=constants.ANGULAR_STEP,
        ...
        self.velocity.calculate(angle, speed)
        for s in game_map._all_ships():
            if self.id == s.id :
                continue

            if custom.euclidean(self, s) <= constants.COLLISON_RANGE:
                c, t = custom.collisionTime(self, s, constants.SHIP_RADIUS/2)
                check = 1
                while c and t >= 0 and t <= 1 and check <= 360/adj:
                    if check == 1:
                        logging.info("Ship "+str(self.id)+" will collide with ship "+str(s.id))
                    angle += adj
                    self.velocity.calculate(angle, speed)
                    c, t = custom.collisionTime(self, s, constants.SHIP_RADIUS/2)
                    check += 1
    ...
    def _parse_single(player_id, tokens):
        ...
        velocity = custom.Velocity(float(x), float(y))
        ...
        ship = Ship(player_id,
                    ...
                    int(progress), int(cooldown), velocity)
