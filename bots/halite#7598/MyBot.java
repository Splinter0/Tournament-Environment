...
public class MyBot {
	public static final String BOT_NAME = "HTBot-2v2";
	
	public static final int PLANET_TOO_FAR = 230, LOCK_ON_SIGHT = 60, PILLAGE_SIGHT = 50, CHALLENGER_SIGHT = 25,
			SUPPORTER_SIGHT = 25, AGGRESSION_SIGHT = 60, DISTRESS_DISTANCE_DOCKED = 25,
			DISTRESS_SIGHT = 25, AUTO_RUSH2P = 85, NUM_SHIPS_BEFORE_HIDE = 20, ENEMY_PLANET_CLOSE_DIST = 15,
			SQUAD_NEARBY = 17, NUM_CORRECTIONS = 36, RUSH4P_DIST = 70, GOOD_PLANET_SCORE_4P = 630;
	public static final boolean ENABLE_4P_RUSH = true;
	public static final double FUDGE_FACTOR = 0.51, WEAPON_RADIUS = 4.999, TEAM_FUDGE = 1.00,
	...
	public static boolean ENABLE_2V2 = true;
	...
	private int[] numShipsProduced = {0, 0, 0, 0};
	private boolean[] stillIn = {true, true, true, true};
	private int highestIdLastTurn = -1;
	...
	private void init() {
		...
		if (isTwoPlayer) {
			...
			stillIn = new boolean[] {true, true};
			numShipsProduced = new int[] {0, 0};
			team_id = -2;
		}
		else {
			LOCK_ON_GOOD_PLANET_SCORE = GOOD_PLANET_SCORE_4P;
			if (ENABLE_2V2) {
				team_id = (gameMap.getMyPlayerId() + 2) % 4;
			}
			else {
				team_id = -2;
			}
		...
		if (isTwoPlayer) {
			...
			for (Ship ship : gameMap.getAllShips()) {
				if (ship.getOwner() != gameMap.getMyPlayerId()) {
					for (Planet p : gameMap.getAllPlanets().values()) {
						if (p.getRemainingProduction() <= 0) continue;
			...
			for (Player p: gameMap.getAllPlayers()) {
				if (p.getId() == gameMap.getMyPlayerId() || p.getId() == team_id ) continue;
	...
	private void nextTurn() {
		...
		if (!clearedForDock) {
			...
			for (Ship mine: gameMap.getMyPlayer().getShips().values()) {
				...
				for (Ship enemy: gameMap.getAllShips()) {
					if (enemy.getOwner() == gameMap.getMyPlayerId() || enemy.getOwner() != team_id ) continue;
		...
		int[] numClumps = {0, 0, 0, 0};
		int highest = highestIdLastTurn;
		for (Ship s : gameMap.getAllShips()) {
			if (s.getId() > highestIdLastTurn) {
				numShipsProduced[s.getOwner()]++;
				if (s.getId() > highest)
					highest = s.getId();
			}
			// Clump.add(s);
			if (s.getOwner() != gameMap.getMyPlayerId() && s.getOwner() != team_id ) {
				int foundKey = -1;
				for (Ship s2: gameMap.getAllShips()) {
					if (s == s2) continue;
					if (s.getDistanceTo(s2) < SUPPORTER_SIGHT) {
						if (s2.getOwner() != gameMap.getMyPlayerId() && s2.getOwner() != team_id && foundKey == -1) {
				...
				if (foundKey == -1) {
					...
					numClumps[s.getOwner()]++;
				}
			}
		}
		highestIdLastTurn = highest;
		...
		for (Planet p : gameMap.getAllPlanets().values()) {
			...
			else if (p.isOwned() && p.getOwner() != team_id) {
				enemyPlanetsCount++;
			}
			
			if (p.getRemainingProduction() > 0) {
				noProdLeft = false;
			}
		}
		...
		if (!isTwoPlayer && ((
				...
				|| (myShipsCount * 6 < (gameMap.getAllShips().size() - myShipsCount) && turnNum > 75) ||
				   (myShipsCount * 5.5 < (gameMap.getAllShips().size() - myShipsCount) && turnNum > 75 && ratioToCenter < 0.3))) {
			totalAvoidMode = true;
		}
		noAggression = false;
		...
		for (Player p: gameMap.getAllPlayers()) {
			if (p.getShips().values().size() == 0) {
				stillIn[p.getId()] = false;
			}
		}
		focusAttack = -1;
		if (noProdLeft || totalAvoidMode) {
			for (int i = 0; i < numShipsProduced.length; i++) {
				if (i == gameMap.getMyPlayerId() || i == team_id) continue;
				if (stillIn[i] && numShipsProduced[i] >= numShipsProduced[gameMap.getMyPlayerId()]) {
					int numShipsNow = gameMap.getAllPlayers().get(i).getShips().values().size();
					int myShipsNow = gameMap.getMyPlayer().getShips().values().size();
					if (myShipsNow >numShipsNow * 0.75 || (myShipsNow > numShipsNow * 1.5 && numClumps[i] > numShipsNow / 3.0)) {
						focusAttack = i;
						break;
					}
				}
			}
			if (focusAttack == -1 && noProdLeft) {
				totalAvoidMode = myShipsCount / (double)gameMap.getAllShips().size() < 0.25;
			}
			else if (focusAttack != -1){
				totalAvoidMode = false;
			}
		}
	}
	...
	private void moveShip(Ship ship, int i, boolean totalDestructionMode) {
		...
		if (ship.getDockingStatus() != Ship.DockingStatus.Undocked) {
			...
			else if (gameMap.getPlanet(ship.getDockedPlanet()).getRemainingProduction() <= 0) {
				moveList.add(new UndockMove(ship));
				didShipsTurn.put(ship.getId(), true);
			}
			return;
		...
		if (focusAttack != -1) {
			for (EntityNode curr = sortedEntities; curr != null; curr = curr.getNext()) {
				if (curr.getEntity() instanceof Ship && curr.getEntity().getOwner() == focusAttack) {
					if (lockOn(ship, curr.getEntity(), false)) {
						return;
					}
				}
			}
		...
		if (!isLeader(ship) || closeSupporters < challengers * 2) {
			...
			if (closeDocked != null && ship.getDistanceTo(closeDocked) < 5 && reallyCloseEnemy != null && closeDocked.getDistanceTo(reallyCloseEnemy) < DISTRESS_DISTANCE_DOCKED) {
				...
				if (closeDocked.getDistanceTo(reallyCloseEnemy) > WEAPON_RADIUS)
					targetPos = createTarget(closeDocked, closeDocked.orientTowardsInRad(reallyCloseEnemy), 3);
				final ThrustMove newThrustMove = nav(ship, targetPos, Constants.MAX_SPEED, NUM_CORRECTIONS,
		...
		if (!squadsNearby(ship, SQUAD_NEARBY)) {
			...
			if (leaderShip == null || leaderShip.getDockingStatus() != DockingStatus.Undocked) {
				...
				if ((leader.target == null || subtractFromFollowShip(leader.target))
						&& lockOn(ship, leader.newestPosition, challengers + supporters, false, 1.1, Constants.MAX_SPEED, null)) {
				...
				int maxThrust;
				if (myDist > leaderDist + Constants.MAX_SPEED) {
					...
					maxThrust = (int)Math.min(Constants.MAX_SPEED, Math.round(myDist - leaderDist));
					if (maxThrust <= 0) {
		...
		if (destructorMode && !noAggression && target != null && target.getOwner() != gameMap.getMyPlayerId() && target.getOwner() != team_id ) {
		...
		if (supporters >= 2 || challengers < 1 + supporters) {
			...
			if (!noAggression && isLeader(ship)) {
				...
				Ship closestDocked = getClosestDockedEnemyShip(ship, Constants.MAX_SPEED, tooManyProtectors);
	...
	private boolean battle(Ship ship, Planet myClosestPlanet, int supporters, int closeSupporters, int challengers,
		...
		if (closestDocked != null && ((closeSupporters > challengers + 2 && closestDocked.getDistanceTo(ship) < Constants.MAX_SPEED + WEAPON_RADIUS) || 
		...
		/*int maxThrust = Constants.MAX_SPEED;
		...
		if (closestCoordAttack != null && ship.getDistanceTo(closestCoordAttack) < Constants.MAX_SPEED + WEAPON_RADIUS && (attackSingle(ship, targetShip) || lockOnMagnet(ship, closestCoordAttack, WEAPON_RADIUS - 1.5))) {
	...
	private boolean isCloseShipBeingAttacked(Ship ship) {
		for (Ship s : gameMap.getMyPlayer().getShips().values()) {
			...
			if (s.getDistanceTo(ship) < Constants.MAX_SPEED) {
				for (Ship s2 : gameMap.getAllShips()) {
					if (s2.getOwner() != gameMap.getMyPlayerId() && s2.getOwner() != team_id && s2.getDistanceTo(p) < WEAPON_RADIUS) {
						return true;
					}
				}
			}
	...
	private boolean regroup(Ship ship, double startTotalX, double startTotalY, double startCount, int dockedMultiplier, boolean avoidEnemy, int distance, int supporters) {
		...
		for (EntityNode curr = sortedEntities; curr != null; curr = curr.getNext()) {
			if (curr.getEntity() instanceof Ship) {
				...
				if (s.getOwner() == ship.getOwner() && dist < distance) {
					if (s.getDockingStatus() != DockingStatus.Undocked && dist < Constants.MAX_SPEED) {
		...
		int thrust;
		if (dist > 15)
			thrust = Constants.MAX_SPEED;
		else if (dist > 10)
			thrust = Constants.MAX_SPEED - 1;
		else if (dist > 7)
			thrust = Constants.MAX_SPEED - 2;
		else if (dist > 5)
			thrust = Constants.MAX_SPEED - 3;
		else if (dist > 3)
			thrust = 3;
		else thrust = 2;
		
		if (avoidEnemy) {
			tm = navAvoidEnemy(ship, Constants.MAX_SPEED, NUM_CORRECTIONS, ANGLE_DELTA, ship.orientTowardsInRad(average), 0, dist, -1);
		}
		else
	...
	private boolean lureEnemy(Ship ship, Ship closestEnemy) {
		...
		if (mine != null && (closestEnemy.getDistanceTo(mine) < 10 || ship.getDistanceTo(mine) < 10)) {
			tm = nav(ship, mine, Constants.MAX_SPEED, NUM_CORRECTIONS, ANGLE_DELTA, 1.2, false);
			if (tm != null) {
				submitThrustMove(tm);
				return true;
			}
		...
		if (tm == null) {
			tm = navAvoidEnemy(ship, Constants.MAX_SPEED, NUM_CORRECTIONS, ANGLE_DELTA, angleRad, 0, 10, -1);
		}
		if (tm != null) {
			submitThrustMove(tm);
			return true;
	...
	private boolean sneakyPillage(Ship ship) {
		...
		if (target == null)
			return false;
		ThrustMove tm = navAvoidEnemy(ship, Constants.MAX_SPEED, NUM_CORRECTIONS, ANGLE_DELTA, ship.orientTowardsInRad(target), 0,
				10, -1);
		if (tm != null) {
			submitThrustMove(tm);
			return true;
	...
	private Position getSneakyPillageTarget(Ship ship) {
		...
		for (EntityNode curr = sortedEntities; curr != null; curr = curr.getNext()) {
			...
			if (e == null || !(e instanceof Ship) || e.getOwner() == gameMap.getMyPlayerId() || e.getOwner() == team_id || ((Ship)e).getDockingStatus() == DockingStatus.Undocked)
				continue;
	...
	private boolean tryPillage(final Ship ship, Ship toPillage, int turnNum, double numPillaging, double undockedShips,
		...
		if (numPillaging / undockedShips < 0.4
				&& !isProtected(toPillage, Constants.MAX_SPEED, closeSupporters+2)
				&& (gameMap.getMyPlayer().getShips().values().size() > 5 || noDockShip == ship.getId())
				&& toPillage.getDockingStatus() != DockingStatus.Undocked
				&& ((turnNum > 30 && dist < PILLAGE_SIGHT) || (dist < PILLAGE_SIGHT / 6)
						|| (noDockShip == ship.getId() && dist < startPillageDistance))
				&& ((goodPlanet == null || goodPlanetScore < LOCK_ON_GOOD_PLANET_SCORE
						|| noDockShip == ship.getId() || (isLeader(ship) && !isProtected(toPillage, Constants.MAX_SPEED, Math.max(2, getLeader(ship).shipsFollowing-1)))))) {
			if (dist < WEAPON_RADIUS) {
				didShipsTurn.put(ship.getId(), true);
				return true;
			}
			ThrustMove tm = navAvoidEnemy(ship, Constants.MAX_SPEED, NUM_CORRECTIONS, ANGLE_DELTA,
	...
	private void findPlanetToPopulate(final Ship ship, Planet[] planets, Planet goodPlanet, double goodPlanetScore,
		...
		for (final Planet planet : planets) {
			if (planet == null)
				break;
			if (planet.getRemainingProduction() <= 0) continue;
			if (planet.isOwned() && planet.getOwner() != gameMap.getMyPlayerId()) {
				continue;
			}

			if (ship.canDock(planet) && !planet.isFull() && planet.getRemainingProduction() > 0) {
	...
	private boolean helpShipInDistress(final Ship ship) {
		...
		if (closestDistress != null) {
			...
			if (closestDistress.caller.getDistanceTo(closestDistress.enemy) > Constants.MAX_SPEED)
				target = createTarget(closestDistress.caller, closestDistress.caller.orientTowardsInRad(closestDistress.enemy), 3);
			final ThrustMove newThrustMove = nav(ship, target, (int)Math.min(Constants.MAX_SPEED, ship.getDistanceTo(target)), NUM_CORRECTIONS,
					ANGLE_DELTA, 1.2, false);
			if (newThrustMove != null) {
				submitThrustMove(newThrustMove);
	...
	private void makeAnyMove(final Ship ship, int prevMoveListSize, Planet[] planets, Ship[] enemyShipsClosest,
		...
		for (final Planet planet : planets) {
			if (planet == null)
				break;
			if (planet.isOwned() && planet.getOwner() != gameMap.getMyPlayerId() && planet.getOwner() != team_id ) {
	...
	private boolean killBestValueShip(Ship ship) {
		...
		Ship best = null;
		for (EntityNode curr = sortedEntities; curr != null; curr = curr.getNext()) {
			if (curr.getDistance() <= Constants.MAX_SPEED && curr.getEntity() instanceof Ship && curr.getEntity().getOwner() != gameMap.getMyPlayerId() && curr.getEntity().getOwner() != team_id ) {
				Ship s = (Ship) curr.getEntity();
	...
	private void hide(Ship ship, int supporters, Ship reallyCloseEnemy) {
		...
		if (ship.getDistanceTo(target) < 4) {
			...
			if (reallyCloseEnemy != null) {
				if (!retreat(ship, reallyCloseEnemy, supporters)) {
					//could not retreat
					moveList.add(new ThrustMove(ship, 0, 0));
					
					DebugLog.addLog("Could not retreat in corner");
					return;
				} else {
					DebugLog.addLog("Hiding ship retreated");
					return;
				}
			}
			//If already in corner don't move.
			moveList.add(new ThrustMove(ship, 0, 0));
			didShipsTurn.put(ship.getId(), true);
			return;
		}
	...
	private boolean performRush4p(Ship ship) {
		...
		if (enemy == null || enemy.getShips().isEmpty() || enemy.getId() == team_id) {
			rush4p = false;
			rush4pTarget = -1;
			return false;
		}
		...
		if (target != null && ship.getDistanceTo(target) < 119) {
			if ((rushRetreat && target.getDockingStatus() == DockingStatus.Undocked) || (!rushAdvance && target.getDockingStatus() == DockingStatus.Undocked && ship.getDistanceTo(target) < Constants.MAX_SPEED + WEAPON_RADIUS && enemy.getShips().values().size() >= gameMap.getMyPlayer().getShips().values().size())) {
				Position target2 = createTarget((Position)ship, target.orientTowardsInRad(ship), 8);
				int thrust = (int)(Math.min(Constants.MAX_SPEED, Math.max(1, -ship.getDistanceTo(target) + 5 + WEAPON_RADIUS)));
			...
			int maxThrust = Constants.MAX_SPEED;
			if (ship.getDistanceTo(target) < WEAPON_RADIUS + Constants.MAX_SPEED) maxThrust = 4;
	...
	private boolean performRush2p(Ship ship) {
		...
		if (turnNum == 0 && gameMap.getMyPlayer().getShips().values().size() == 3 && Math.abs(ship.getXPos() - gameMap.getWidth() / 2.0) < 5) {
			Position target = null;
			...
			ThrustMove tm = nav(ship, target, Math.min(Constants.MAX_SPEED, (int)Math.ceil(ship.getDistanceTo(target))), NUM_CORRECTIONS, ANGLE_DELTA, 0, false, 0);
			if (tm != null) {
				submitThrustMove(tm);
				return true;
			}
		...
		if (!allClose && closest != null && gameMap.getAllShips().size() - myShipsCount >= myShipsCount) {
			if (ship.getDistanceTo(closest) > 3) {
				...
				int thrust = Math.min(Constants.MAX_SPEED, (int)ship.getDistanceTo(closest));
				...
				int thrust = Math.min(Constants.MAX_SPEED, (int)ship.getDistanceTo(farthest));
				if (farthest instanceof Ship) thrust = Math.min(Constants.MAX_SPEED, (int)(ship.getDistanceTo(farthest)/2.0));
		...
		if (!wait) {
			...
			if (target2 != null) {
				...
				else if ((rushRetreat && target2.getDockingStatus() == DockingStatus.Undocked) || (!rushAdvance && closestEnemy != null && target2.getDockingStatus() == DockingStatus.Undocked && ship.getDistanceTo(closestEnemy) < Constants.MAX_SPEED + WEAPON_RADIUS  && gameMap.getAllShips().size() - myShipsCount >=  myShipsCount)) {
					Position target = createTarget((Position)ship, closestEnemy.orientTowardsInRad(ship), 8);
					int thrust = (int)(Math.min(Constants.MAX_SPEED, -ship.getDistanceTo(closestEnemy) + 5 + WEAPON_RADIUS));
				...
				if ((target2.getDockingStatus() != DockingStatus.Undocked || ship.getHealth() > target2.getHealth()) || subtractFromFollowShip(target2)) {
					/*int maxThrust = Constants.MAX_SPEED;
					if (ship.getDistanceTo(target2) < WEAPON_RADIUS + Constants.MAX_SPEED) maxThrust = 4;
	...
	private double performTotalDestruction(double numPillaging, final Ship ship, int supporters, int challengers) {
		for (EntityNode curr = sortedEntities; curr != null; curr = curr.getNext()) {
			Entity e = curr.getEntity();
			if ((e instanceof Ship) && e.getOwner() != gameMap.getMyPlayerId() && e.getOwner() != team_id && ((Ship)e).getDockingStatus() != DockingStatus.Undocked) {
				...
				Planet p = (Planet) e;
				if ((!p.isOwned() || p.getOwner() == gameMap.getMyPlayerId()) && ship.canDock(p) && !p.isFull() && supporters > challengers + 2 && p.getRemainingProduction() > 0) {
	...
	private boolean isProtected(Ship ship, double dist, int numProtectors) {
		...
		for (Ship s : gameMap.getAllShips()) {
			if (s.getOwner() != ship.getOwner() ||
				...
				continue;
			Entity obj = getObjectBetween(s, ship, 1, enemyPlanets, Constants.MAX_SPEED, false);
			if (obj == null)
				num++;
	...
	private Ship getClosestEnemy(Position closest2) {
		...
		double minDist = Double.MAX_VALUE;
		for (Ship s2 : gameMap.getAllShips()) {
			if (((closest2 instanceof Ship && s2.getOwner() != ((Ship)closest2).getOwner()) || s2.getOwner() != gameMap.getMyPlayerId()) && s2.getOwner() != team_id ) {
	...
	private Planet getClosestPlanet(Position closest2) {
		Planet closest = null;
		double minDist = Double.MAX_VALUE;
		for (Planet p2 : gameMap.getAllPlanets().values()) {
			{
				double d = closest2.getDistanceTo(p2);
				if (d < minDist) {
					minDist = d;
					closest = p2;
				}
	...
	private double scorePlanet(Planet p, double distanceToShip) {
		...
		for (Planet p2 : gameMap.getAllPlanets().values()) {
			if (p2 == p)
				continue;
			if (p2.isOwned()) {
				if (p2.getOwner() != gameMap.getMyPlayerId() && p2.getOwner() != team_id && p.getDistanceTo(p2) < 40) {
					enemyPlanetsNearby++;
				continue;
				}
			} else if (isEarlyGame) {
				for (Ship s : gameMap.getAllShips()) {
					...
					if (getClosestPlanet(s).getId() == p2.getId()) {
						enemyPlanetsNearby++;
					}
				}
			}
			if (p.getDistanceTo(p2) < 50 && p.getRemainingProduction() <= 0)
				emptyPlanetsNearby++;
		}
		...
		double score = (400 - 3.3 * (distanceToShip)) + ratioToCenter * 50 + 30 * p.getRadius() + emptyPlanetsNearby * emptyPlanetsMult * 25 - enemyPlanetsNearby * 15 - penalty;
		return score;
	}
	...
	private boolean checkDistressCalls(Collection<Ship> myShips, boolean enemyOwnsPlanet) {
		...
		for (Ship ship : myShips) {
			...
			for (Ship ship2 : gameMap.getAllShips()) {
				if (ship2.getOwner() == gameMap.getMyPlayerId() || ship2.getOwner() == team_id  || ship2.getDockingStatus() != DockingStatus.Undocked)
					continue;
				double dist = ship.getDistanceTo(ship2);
				boolean isWithinDist = dist < DISTRESS_DISTANCE_DOCKED;
				
				if (ship2.getOwner() != gameMap.getMyPlayerId() && ship2.getOwner() != team_id && isWithinDist) {
					count++;
	...
	private int countEnemyPlanets() {
		int planets = 0;
		for (Planet p : gameMap.getAllPlanets().values()) {
			if (p.isOwned() && p.getOwner() != gameMap.getMyPlayerId() && p.getOwner() != team_id ) {
				planets++;
			}
		}
		if (planets == 0) {
			for (Ship s : gameMap.getAllShips()) {
				if (s.getOwner() != gameMap.getMyPlayerId() && s.getOwner() != team_id && s.getDockingStatus() != DockingStatus.Undocked) {
					planets = 1;
					break;
				}
			}
	...
	private Entity getObjectBetween(Position start, Position target, double fudge, ArrayList<Entity> entities,
		...
		for (Entity e : entities) {
			...
			if (e instanceof MovingShip) {
				...
				if (e instanceof Ship && e.getOwner() == team_id && Collision.segmentCircleIntersect(start, target, e, fudge + TEAM_FUDGE, false))
					return e;
	...
	private Entity getObjectBetweenExtensive(Position start, Position target, double fudge, ArrayList<Entity> entities,
		...
		for (Entity e : entities) {
			...
			if (e instanceof MovingShip) {
				...
				if (e instanceof Ship && ((Ship)e).getOwner() == team_id &&
						Collision.segmentCircleIntersect(start, target, e, fudge + TEAM_FUDGE, false))
					return e;
				else if (Collision.segmentCircleIntersect(start, target, e, fudge, false))
					return e;
			}
		}
	...
	private boolean canCollideWithLine(Ship ship, Position start, Position end, int thrust, double fudge, boolean assumeMoveStraight) {
		if (assumeMoveStraight) {
			...
			for (int newThrust = 0; newThrust <= Constants.MAX_SPEED; newThrust++) {
		...
		for (int i = 0; i <= Constants.MAX_SPEED * 2; i++) {
			double t = i / (double)(Constants.MAX_SPEED * 2) * thrust;
			Position p = new Position(start.getXPos() + t * dx, start.getYPos() + t * dy);
			if (ship.getDistanceTo(p) < WEAPON_RADIUS + fudge + i / (double)(Constants.MAX_SPEED * 2) * Constants.MAX_SPEED)
				return true;
		}
		return false;
	}
	...
	private boolean canReachEnemy(Ship ship, Ship target) {
		...
		if (ship.getDistanceTo(target) < Constants.MAX_SPEED) thrust = (int)(ship.getDistanceTo(target) - 0.8);
	...
	private boolean lockOn(Ship ship, Position target, boolean kamikaze) {
		return lockOn(ship, target, 0, kamikaze, kamikaze ? 0 : WEAPON_RADIUS / 3.0 * 2.0, Constants.MAX_SPEED, null);
	}

	private boolean lockOn(Ship ship, Position target, int numEnemies, int numSupporters) {
		return lockOn(ship, target, numEnemies + numSupporters, /* kamikaze */false, WEAPON_RADIUS / 3.0 * 2.0, Constants.MAX_SPEED, null);
	}
	...
	private boolean lockOn(Ship ship, Position target, int numShipsAround, boolean kamikaze, double gap, int maxThrust, ArrayList<Ship> group, boolean adjustPosIfNecessary) {
		...
		if (target instanceof Entity && (((Entity)target).getOwner() == gameMap.getMyPlayerId() || ((Entity)target).getOwner() == team_id )) {
		...
		if (target instanceof Ship && !kamikaze) {
			...
			if (myClosestDockedShip != null && target.getDistanceTo(myClosestDockedShip) < 10) {
				...
				int numCorrections = ship.getDistanceTo(target) < Constants.MAX_SPEED ? 3 : 2;
	...
	private boolean lockOnMagnet(Ship ship, Position target, double gap, int maxThrust, ArrayList<Ship> group) {
		...
		Ship enemyShip = null;
		if (target instanceof Ship && ((Ship)target).getOwner() != gameMap.getMyPlayerId() && ((Ship)target).getOwner() != team_id && ((Ship)target).getId() >= 1) {
			enemyShip = (Ship)target;
		}
		else {
			enemyShip = getClosestEnemy(ship);
		}
		if (ship.getDistanceTo(enemyShip) > Constants.MAX_SPEED) {
			...
			if (ms != null && ms.posAfterThisTurn != null && ms.thrust != 0 && ms.posAfterThisTurn.getDistanceTo(pivot) < 5 && isWithinRad(angleRad, ship.orientTowardsInRad(ms.posAfterThisTurn), Math.PI / 6.0)) {
				...
				if (wouldBeThrust <= Constants.MAX_SPEED && (wouldBeThrust >= 3 || maxThrust <= 3)) {
					...
					for (int i = 2; i <= 6; i++) {
						for (int j = -1; j <= 1; j += 2) {
							...
							if (ship.getDistanceTo(try1) < 7.6) {
								int thrust = (int)Math.min(Constants.MAX_SPEED, Math.round(ship.getDistanceTo(try1)));
	...
	private ArrayList<Entity> getEntitiesCloseTo(final Ship ship, final double buffer, ArrayList<Ship> dontAdd) {
		...
		for (EntityNode curr = sortedEntities; curr != null; curr = curr.getNext()) {
			...
			if (d < ship.getRadius() + e.getRadius() + Constants.MAX_SPEED + buffer) {
		...
		for (MovingShip ms: movingShips) {
			...
			if (ms.getDistanceTo(ship) < ship.getRadius() + ms.getRadius() + Constants.MAX_SPEED + ms.thrust + buffer) {
	...
	private ThrustMove navSingleTargetGroup(Ship ship, Ship target, ArrayList<Ship> group) {
		...
		if (ship.getDistanceTo(target) > WEAPON_RADIUS + Constants.MAX_SPEED) return null;
		...
		return navSingleTargetGroup(ship, target, Constants.MAX_SPEED, NUM_CORRECTIONS, ANGLE_DELTA,
	...
	private ThrustMove navToDock(final Ship ship, final Planet dockTarget) {
		if (ship.getDistanceTo(dockTarget) > 8 + dockTarget.getRadius()) {
			...
			return nav(ship, targetPos, Constants.MAX_SPEED, NUM_CORRECTIONS, ANGLE_DELTA, 0, false);
		}
		final double buffer = 5;
	...
	private Position getClosestPoint(final Ship ship, final Planet target) {
		...
		for (Planet p: gameMap.getAllPlanets().values()) {
			...
			if (p.isOwned() && p.getOwner() != gameMap.getMyPlayerId() && p.getOwner() != team_id && dist < minDist && dist < ENEMY_PLANET_CLOSE_DIST) {
				minDist = dist;
		...
		public Surroundings(MyBot myBot, Ship ship, GameMap gameMap, EntityNode entities) {
			...
			for (EntityNode curr = entities; curr != null; curr = curr.getNext()) {
				...
				if (e instanceof Planet) {
					Planet p = (Planet) e;
					planets[planetI++] = p;
					if (p.isOwned() && p.getOwner() != gameMap.getMyPlayerId()  && p.getOwner() != myBot.team_id) {
					...
					Ship ship1 = (Ship) e;
					if (!foundTarget && ship1.getOwner() != gameMap.getMyPlayerId() && ship1.getOwner() != myBot.team_id) {
						if (d < LOCK_ON_SIGHT) {
							foundTarget = true;
							target = ship1;
						}
					...
					if (ship1.getOwner() != gameMap.getMyPlayerId() && ship1.getOwner() != myBot.team_id) {
						enemyShipsClosest[enemyI++] = ship1;
						if (ship1.getDockingStatus() == DockingStatus.Undocked && d < CHALLENGER_SIGHT) {
							if (d < Constants.MAX_SPEED + WEAPON_RADIUS && reallyCloseEnemy == null)
							...
							challengers++;
							if (d < Constants.MAX_SPEED) closeChallengers++;
						}
						...
						if (c == null || !clumpsChecked.contains(c)) {
							...
							for (Ship ship2: shipsToCheck) {
								...
								if (willShipDock == null || willShipDock == false) {
									supporters += 1;
									if (c.isAnyWithin(ship, Constants.MAX_SPEED))
										closeSupporters += 1;
								}
							}
							clumpsChecked.add(c);
						...
						if (willShipDock == null || willShipDock == false) {
							supporters++;
							if (d < Constants.MAX_SPEED) closeSupporters++;
						}
					} else if (ship1.getOwner() == gameMap.getMyPlayerId() && d < SUPPORTER_SIGHT) {
						protecting++;
					}
